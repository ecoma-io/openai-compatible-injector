...
func TestRequestCompletedLogLifecycle(t *testing.T) {
	...
	h := NewHandler(promptStore(t, upstream.URL, "SECRET_PROMPT_VALUE inject me", testAPIKey), directResolver(), nil, nil, nil, log)
...
func TestRequestCompletedOutcomes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			buf, log := captureLog(zerolog.InfoLevel)
			h := NewHandler(newTestStore(t, "http://127.0.0.1:1/v1"), directResolver(), nil, nil, nil, log)
...
func TestStreamTruncationPhaseLogging(t *testing.T) {
	t.Run("client_write", func(t *testing.T) {
		...
		buf, log := captureLog(zerolog.InfoLevel)
		h := NewHandler(newTestStore(t, upstream.URL), directResolver(), nil, nil, nil, log)
	...
	t.Run("upstream_read", func(t *testing.T) {
		...
		h := NewHandler(newTestStore(t, "http://127.0.0.1:1/v1"), fixedDoer{client}, nil, nil, nil, log)
...
func TestRelayOutcomeClassification(t *testing.T) {
	verbatim := func(body io.ReadCloser) (buf *logBuffer, h http.Handler) {
		...
		return buf, NewHandler(newTestStore(t, "http://127.0.0.1:1/v1"), fixedDoer{client}, nil, nil, nil, log)
	}
	...
	t.Run("buffered_write_error", func(t *testing.T) {
		...
		buf, log := captureLog(zerolog.InfoLevel)
		h := NewHandler(promptStore(t, upstream.URL, "prompt", testAPIKey), directResolver(), nil, nil, nil, log)
		dying := &dyingRecorder{limit: 0}
		req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(`{"model":"test-model"}`))
		req.Header.Set("Authorization", "Bearer "+testAPIKey)
		h.ServeHTTP(dying, req)
...
func TestDebugLifecycleChain(t *testing.T) {
	...
	h := NewHandler(promptStore(t, upstream.URL, "SECRET_PROMPT_VALUE inject me", "SECRET_AUTH_VALUE"), directResolver(), nil, nil, nil, log)
...
func TestErrorEnvelopeWriteFailureOutcome(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			h := NewHandler(tc.store(t), fixedDoer{client}, nil, nil, nil, log)
