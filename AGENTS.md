...
prompt into every request. Two model-serving API surfaces: Chat Completions
(`/v1/chat/completions`) and Responses (`/v1/responses`), both with SSE
streaming passthrough; plus authenticated model discovery at `GET /v1/models`,
answered locally from the configured public mapping (never from an upstream).
...
| `internal/credential`            | Per-provider upstream credential configuration and rotation state: validated `auth:`-block `Spec` (safe-charset `Key` ids, secret `Key.Value` kept in memory only, `ContentKey` — a sha256 over the block's bytes INCLUDING the values, never logged; the content identity, not the pool identity), `Provider` (Identity = the providers-table name + Spec + `RateLimit`; `PoolKey()` = sha256 over ALL THREE length-prefixed — two providers with byte-identical credentials are two rotation domains, and a rate-limit-only change moves the key; never logged), `Pool` (round-robin acquire with a sticky preferred key that never moves the cursor, per-key cooldown deadlines with lazy expiry against the caller's clock, `MarkRateLimited`/`NextReady`/`CoolingUntil` — zero time when not cooling, the future deadline when cooling, zero again once expired at the CALLER's now, never a wall clock — inert, no I/O, no goroutines, no timers, no sleeps, no recovery counters; `NewPool` panics on an empty spec because the loader rejects it), and `Registry` (get-or-create by pool key, `Retain` on publish drops removed pools). The package knows nothing about HTTP, recovery, or transport — `guards_test.go` enforces both by parsing the package's own source.                                                                                                                                                                                                                                                                                                                     |
| `internal/inject`                | Pure request/response transforms: `Probe` (model + stream detection), `Chat`, `Responses`, `RewriteChatModel`/`RewriteResponsesModel` (byte-preserving, API-scoped), thinking plan + usage synthesizers (`ThinkingPlanFor`, `SynthesizeChat/ResponsesThinkingUsage`), response field stripping (`StripChatFields`/`StripResponsesFields`, `StripPatterns`, byte-preserving via the shared splice, object-only traversal)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `internal/recovery`              | The provider recovery policy domain, deliberately free of I/O and of HTTP: typed `Failure` classes with closed-set cause tokens, typed allow-listed `Match` predicates and the three `Action`s (zero value `terminal`), the failure→action matrix with canonical rule IDs, specificity-derived precedence and equal-specificity overlap rejection (`matrix.go`), the shipped default policy (`defaults.go`), the layer merge (`merge.go`: pointer-typed `Partial`s — scalars replace, maps deep-merge, same-ID rule replaces, new ID appends, unstated inherits) and `Resolve` (validate after every layer, so no layer can smuggle a broken policy past the merge), the policy's stable data hash (`hash.go`, hand-written field order, never map iteration), and the `Engine` (`engine.go`/`budget.go`/`retryafter.go`/`backoff.go`) turning one `Observation` into one `Decision` under the four code-owned invariants — commitment, caller, envelope, immutability. Blocked-credential state reaches the engine ONLY as input to a Decision: one credential-cooldown failure class folded through `Observe`, carrying the pool's earliest-ready remainder as `Observation.CredentialReadyIn` — a LOCAL readiness floor applied to every wait outside the `RetryAfter` channel (the retry-after policy's mode/ceilings never erase it; the retry window, caller deadline and envelopes still bind it) — never a fake response, never a decision the engine did not make. It never sleeps, dials, writes a response, reads a pool, or logs; the proxy owns execution and the transport owns the wire |
| `internal/proxy`                 | HTTP handler wiring, client bearer authentication/Authorization stripping (auth delegated to the `auth.Provider` seam — static or partner), local authenticated `GET /v1/models` discovery (the request's one snapshot only; public mapping keys sorted; no upstream call; model entries exactly `id` + `created: 0`; 405-before-401), error envelopes, SSE copying (`CopySSE`), the provider candidate walk, orchestrated THROUGH the `recovery.Engine` — the handler contributes facts (which candidate, which attempt, what the attempt produced, whether the client is still there and whether a byte already reached it) and executes the returned decision (wait the decision's bounded delay, enter the next candidate, or relay what it holds), never classifying a status itself; the walk may not start at all until the snapshot's frozen policy says so, and no decision is taken outside the engine. Executes upstream calls through the model's resolved `transport.Doer` — the `Executor` (pool) branch handed the request's `recovery.Budget` and its `egress_kind`/`egress_target`/`egress_exhausted` report folded into the evidence — composed response rewriter (`rewriteOut`: model rename + thinking-usage synthesis + response field stripping, candidate-aware per-hop `strip-fields`, the strip running last; SSE gate widened with `stripKeys`)                                                                                                                                                                                                                              |
...
- **Client authentication is mandatory and terminal at this proxy.** Every
  `/v1` request, including Chat/Responses and local `GET /v1/models`, presents
  a bearer credential as `Authorization: Bearer <key>`; the scheme is
  case-insensitive, and keys must be RFC 6750 bearer tokens (no whitespace or
  other invalid characters). A missing/malformed key gets the static
  missing-key 401, while a wrong key gets the static `invalid_api_key` 401.
  Authenticate before reading the body or upstream I/O; retain 405-before-401
  ordering. `/healthz` and the 404 catch-all stay unauthenticated. Consume —
  never forward or replace — the client's Authorization header; upstreams are
  trusted/internal.
...
- **Response field stripping is config-data, byte-preserving, and runs
  ...
  own bytes — key, colon, value, comma — via the shared splice, so validated
  input keeps every other byte; unparseable input is returned unchanged
  (same-slice, the SSE pointer-identity fast path depends on it). It runs
  LAST in the composed `rewriteOut` (model rename → thinking synthesis →
  strip), deliberately, so configuration can never strip the `model`/`usage`
  keys the proxy itself writes. The strip list binds per request exactly
  like the rest of the snapshot: a model-level list replaces its
  candidates' provider lists; without one, each candidate answers under its
  own provider's list (`m.Strip` swapped alongside the endpoint in the
  candidate view). The SSE data-line gate is widened with the strip list's
  first-segment byte patterns (`stripKeys`), re-derived per candidate, so a
  chunk carrying only a to-be-excised key still reaches the strip; nil/empty
  patterns keep the unconfigured gate byte-identical. The usage meter
  observes pre-rewrite bytes, so stripped fields are never metered.
...
- **Never log or leak credentials.** No `Authorization`, keys, request
  ...
  provider bodies, and driver errors also never appear. Upstream credential
  VALUES from a provider `auth:` block are the same class of secret: they
  live in the config file, in memory, and on the outgoing wire — never in an
  error, a log line, a reload event, a metric label, a panic, or a response
  body. Only the operator-chosen key `id` from the safe charset
  (`[A-Za-z0-9._:-]`) may travel through logs (`upstream_credential_id`),
  and the pool's content-identity digest (which hashes the values) is never
  logged either. A quote of these is a
...
- 404 `invalid_request_error` — unknown path (no route matched):
  ...
  `GET /v1/models` is exact-match only: `/v1/models/` is this 404.
- 405 `invalid_request_error` — non-GET `/v1/models` has the same exact
  `method not allowed` envelope as wrong-method Chat/Responses, before any
  authentication check.
- 200 `GET /v1/models` — no upstream call; response is built from the request
  snapshot's configured public names, lexicographically sorted:
  `{"object":"list","data":[{"id":"<public-model>","created":0}]}`.
  Every entry has exactly `id` and `created`; the latter is always numeric 0.
  No `object`, `owned_by`, upstream name, endpoint, prompt, or other metadata
