...
- [ ] Trivial — moves no documented behavior (failure classification, reload semantics, injection behavior, streaming, shutdown)
- [ ] Contract-bearing — the affected README/AGENTS sections are updated in this same PR

## Could this fail silently?

<!-- The dangerous direction in this repository is the quiet one: an injection
     prompt that stops being applied, a stream that is buffered instead of
     passed through, a request that survives a reload it should not survive.
...
- [ ] No `config.yaml`, credentials, or Authorization values anywhere in the diff
